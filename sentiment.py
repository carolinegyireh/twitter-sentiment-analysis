...
try:
    response = requests.get("https://miro.medium.com/max/1000/1*vp1M37AGMOFwCvLxVm62IA.jpeg")
    header_image = Image.open(BytesIO(response.content))
    st.image(header_image, use_container_width=True, caption="Twitter Sentiment Analysis")
except Exception as e:
    st.warning(f"Couldn't load header image: {str(e)}")
